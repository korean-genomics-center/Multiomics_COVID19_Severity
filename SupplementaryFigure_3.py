...
list_keep_id = list(set(list_keep_id).difference(set(list_drop_samples)))
...
for i, (marker, ax) in enumerate(zip(list_target_methyl, axes)):
    ...
    pvalues = df_marker_stat_copy.loc[marker, "pval"]
    ...
    list_xticklabels = get_xticklabels(df_beta_selec_target, dict_name_vis={"First": "", "Last": ""}, colsample=colsample)
    if (i // ncols) == (nrows - 1):
        ax.set_xticklabels(list_xticklabels, fontsize=plt.rcParams["font.size"]+4, rotation=45, ha="right", rotation_mode="anchor")
    else:
        ax.set_xticklabels([], fontsize=0, rotation=45, ha="right", rotation_mode="anchor")
    
    ax.tick_params(axis='y', labelsize=plt.rcParams["font.size"]+4)
    
    if i % ncols == 0:
        ax.set_ylabel("Proportion of\nMethylated CpGs (%)", fontsize=plt.rcParams["font.size"]+6)
    else:
        ax.set_ylabel("", fontsize=0)
    markername = f"{marker}\n$\it({dict_marker_target.get(marker, marker)})$\n"
    ax.set_title(markername, fontsize=plt.rcParams["font.size"]+6)
    markername = f"{marker}\n$\it({dict_marker_target.get(marker, marker)})$\n"
    legend_elements = [
...
plt.subplots_adjust(hspace=0.3, wspace=0.5, left=0.1, right=0.9)
plt.savefig(f"{WORKDIR}/Results/Paper/SupplementaryFigure3.pdf", bbox_inches="tight", dpi=300)
plt.savefig(f"{WORKDIR}/Results/Paper/SupplementaryFigure3.png", bbox_inches="tight", dpi=300)
plt.show()
plt.close()
