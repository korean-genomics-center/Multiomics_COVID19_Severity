...
class Parse():
    ...
    def filter_dataframe(self):
        if len(self.list_drop_samples) > 0:
            df_crf_filtered = self.df_crf[~self.df_crf.iloc[:, 0].isin(self.list_drop_samples)]
        else:
    ...
    def add_info_severity_dataframe(self, target="Severity_group"):
        ...
        for sampleid in self.df_crf.iloc[:, 0]:
            if str(sampleid).split("-")[1][0] == "R":
                df_crf_indexed = self.df_crf.set_index(self.df_crf.columns[0])
    ...
    def lookup_dict_sample_visit(self, df_crf):
...
class Plot(): 
    ...
    def edit_xticklabels(self, count_target):
    ...
    def draw_boxplot(self, df_compare, colname):
        flierprops = dict(marker='o', markerfacecolor='None', markersize=10,  markeredgecolor='black')
        order = sorted(list(df_compare[self.target].unique()))
        ...
        g.set_xlabel(self.target,fontsize=0)
        g.set_ylabel(colname,fontsize=20)
        ...
        g.set_xticks(ticks=list(range(0, len(new_xticklabels))), 
                      labels=new_xticklabels, 
                      fontsize=16)
        
        return g, pvalues

    def draw_barplot(self, df_compare, colname):
        series_colname = df_compare.groupby(self.target)[colname].apply(np.array)
